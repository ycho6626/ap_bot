...
class TestDerivativeEndpoint:
    ...
    def test_derivative_positive_case(self):
        ...
        assert data["details"]["symbolic_match"] is True
    ...
    def test_derivative_with_tolerance(self):
        """Test derivative endpoint with custom tolerance."""
        request_data = {
            "expr": "x^2",
            "var": "x",
            "expected": "2*x",
...
class TestIntegralEndpoint:
    ...
    def test_integral_negative_case(self):
        """Test integral endpoint with incorrect integral."""
        request_data = {
            "expr": "2*x",
            "var": "x",
            "expected": "x"
    ...
    def test_integral_without_constants(self):
        """Test integral endpoint without ignoring constants."""
        request_data = {
            "expr": "2*x",
            "var": "x",
            "expected": "x^2 + 5",
...
class TestLimitEndpoint:
    ...
    def test_limit_with_numeric_value(self):
        """Test limit endpoint with numeric limit point."""
        request_data = {
            "expr": "x^2",
            "var": "x",
            "to": 0,
...
class TestAlgebraEndpoint:
    ...
    def test_algebra_with_tolerance(self):
        """Test algebra endpoint with custom tolerance."""
        request_data = {
            "lhs": "x^2 + 2*x + 1",
            "rhs": "(x + 1)^2",
            "tolerance": 1e-15
...
class TestDimensionalEndpoint:
    ...
    def test_dimensional_negative_case(self):
        """Test dimensional endpoint with non-unit expression."""
        request_data = {
            "expr": "x^2 + 1",
            "expected_unit": "m"
        }
...
class TestNumericProbeEndpoint:
    ...
    def test_numeric_probe_negative_case(self):
        """Test numeric probe endpoint with problematic expression."""
        request_data = {
            "expr": "1/0",
            "num_points": 3
        }
...
class TestErrorHandling:
    ...
    def test_missing_required_fields(self):
        ...
        # Test derivative endpoint with missing fields
        request_data = {"expr": "x^2"}  # Missing var and expected
        response = client.post("/calc/derivative", json=request_data)
    ...
    def test_invalid_direction(self):
        """Test limit endpoint with invalid direction."""
        request_data = {
            "expr": "x^2",
            "var": "x",
            "to": "0",
        ...
        response = client.post("/calc/limit", json=request_data)
        assert response.status_code == 200  # Should still work, just use default
        data = response.json()
        assert "equivalent" in data
...
class TestEdgeCases:
    ...
    def test_very_large_expressions(self):
        """Test with very large expressions."""
        request_data = {
            "expr": "x^1000",
            "var": "x",
            "expected": "1000*x^999"
