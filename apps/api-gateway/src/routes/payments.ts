...
import { getValidStripePriceIds, getStripePriceIdsForRole } from '@ap/payments/roles';
...
export const paymentRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/plans', {
    ...
    try {
      ...
      logger.error({ error: error instanceof Error ? error.message : 'Unknown error' }, 'Failed to get pricing plans');
      reply.status(500);
      return {
        error: {
  ...
  fastify.post('/stripe/portal', {
    ...
    try {
      ...
      try {
        ...
        logger.error({ error: error instanceof Error ? error.message : 'Unknown error' }, 'Failed to create/find Stripe customer');
        reply.status(500);
        return {
          error: {
      ...
      logger.error({ error: error instanceof Error ? error.message : 'Unknown error' }, 'Failed to create billing portal session');
      reply.status(500);
      return {
        error: {
