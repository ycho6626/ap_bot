...
import { ExamVariant, ReviewCaseStatus, UserRole } from '@ap/shared/types';
...
export default function ReviewPage() {
  ...
  const [filters, setFilters] = useState({
    status: 'pending' as ReviewCaseStatus,
    examVariant: undefined as ExamVariant | undefined,
    limit: 20,
    offset: 0,
  });
  ...
  const loadCases = async () => {
    try {
      setLoading(true);
      const response = await ReviewApi.getCases(filters);
  ...
  const handleCaseResolve = async (caseId: string, action: 'approve' | 'reject' | 'request_revision', data?: {
    feedback?: string;
