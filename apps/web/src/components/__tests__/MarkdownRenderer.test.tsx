...
describe('MathMarkdownRenderer', () => {
  ...
  it('should handle mixed content with text and math', async () => {
    const content = 'The derivative of $x^2$ is $2x$. Here is the integral:\n\n$$\\int_0^1 x^2 dx = \\frac{1}{3}$$';
    const { container } = render(<MathMarkdownRenderer content={content} />);
  ...
  it('should handle multiple inline math expressions', async () => {
    const content = 'The derivative of $x^2$ is $2x$, and the derivative of $\\sin(x)$ is $\\cos(x)$.';
    const { container } = render(<MathMarkdownRenderer content={content} />);
  ...
  it('should handle complex LaTeX expressions', async () => {
    const content = 'Here is a complex integral:\n\n$$\\int_{-\\infty}^{\\infty} e^{-x^2} dx = \\sqrt{\\pi}$$';
    const { container } = render(<MathMarkdownRenderer content={content} />);
  ...
  it('should handle math in markdown blockquotes', async () => {
    const content = '> The fundamental theorem of calculus states that $\\int_a^b f\'(x) dx = f(b) - f(a)$';
    const { container } = render(<MathMarkdownRenderer content={content} />);
  ...
  it('should apply custom className', () => {
    ...
    const { container } = render(<MathMarkdownRenderer content={content} className="custom-class" />);
    expect(container.firstChild).toHaveClass('custom-class');
  });
