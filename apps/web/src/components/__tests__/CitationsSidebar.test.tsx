...
const mockSuggestions = ['Try asking about the chain rule', 'Learn more about definite integrals'];
...
describe('CitationsSidebar', () => {
  ...
  it('should render source type badges correctly', () => {
    render(<CitationsSidebar sources={mockSources} suggestions={[]} onClose={vi.fn()} />);
  ...
  it('should render suggestions when provided', () => {
    render(<CitationsSidebar sources={[]} suggestions={mockSuggestions} onClose={vi.fn()} />);
  ...
  it('should call onClose when close button is clicked', () => {
    ...
    render(<CitationsSidebar sources={[]} suggestions={[]} onClose={mockOnClose} />);
  ...
  it('should render empty state when no sources or suggestions', () => {
    render(<CitationsSidebar sources={[]} suggestions={[]} onClose={vi.fn()} />);

    expect(
      screen.getByText('No sources or suggestions available for this response.')
    ).toBeInTheDocument();
  });

  it('should apply custom className', () => {
    const { container } = render(
      <CitationsSidebar sources={[]} suggestions={[]} onClose={vi.fn()} className='custom-class' />
    );
