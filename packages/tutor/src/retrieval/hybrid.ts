...
export class HybridRetrieval {
  ...
  async search(
    ...
    this.logger.debug(
      { query, examVariant, limit, minScore },
    ...
    return traceDatabaseOperation(
      ...
      async () => {
        ...
        const results = (data || []).map(doc => {
          const embedding = (doc as any).kb_embedding?.embedding;
          if (!embedding) {
            return null;
          }
   ...
   */
  private async getQueryEmbedding(_query: string): Promise<number[]> {
